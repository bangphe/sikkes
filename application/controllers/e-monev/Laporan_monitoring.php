...
class Laporan_monitoring extends CI_Controller 
{
	...
	function grid()
	{
		...
		$colModel['realisasi_output'] = array('Realisasi Output',90,TRUE,'center',0);
		$colModel['progres_output'] = array('Progress Output',90,TRUE,'center',0);
	...
	function grid_data_monitoring() 
	{
		...
		foreach ($records['records']->result() as $row){
				...
				//monev DJA
				$kdgiatoutput = $row->kdgiat.'.'.$kdoutput;
				$dja = $this->lmm->get_dja($kdgiatoutput);
				if ($dja->num_rows() > 0) {
				 	$realisasi_keuangan = $this->lmm->get_dja($kdgiatoutput)->row()->realisasi_anggaran;
				 	$realisasi_output = $this->lmm->get_dja($kdgiatoutput)->row()->realisasi_output;
				 	$progres_output = $this->lmm->get_dja($kdgiatoutput)->row()->progres_output;
				 	$alokasi = $this->lmm->get_dja($kdgiatoutput)->row()->pagu_anggaran;
				} else {
					$realisasi_keuangan = 0;
					$realisasi_output = 0;
					$progres_output = 0;
				...
				//warning icon realisasi keuangan
				if($realisasi_keuangan < 50)
				{
					$warning_icon_keuangan = '<img border=\'0\' src=\''.base_url().'images/flexigrid/bulb_red.png\'>';
				}
				else if($realisasi_keuangan >= 50 && $realisasi_keuangan < 75)
				{
					$warning_icon_keuangan = '<img border=\'0\' src=\''.base_url().'images/flexigrid/bulb_yellow.png\'>';
				}
				else if($realisasi_keuangan >= 75 && $realisasi_keuangan <= 100)
				{
					$warning_icon_keuangan = '<img border=\'0\' src=\''.base_url().'images/flexigrid/bulb_green.png\'>';
				}
				else if($realisasi_keuangan > 100)
				{
					$warning_icon_keuangan = '<img border=\'0\' src=\''.base_url().'images/flexigrid/bulb_blue.png\'>';
				}

				//warning icon realisasi keuangan
				if($realisasi_output < 50)
				{
					$warning_icon_output = '<img border=\'0\' src=\''.base_url().'images/flexigrid/bulb_red.png\'>';
				}
				else if($realisasi_output >= 50 && $realisasi_output < 75)
				{
					$warning_icon_output = '<img border=\'0\' src=\''.base_url().'images/flexigrid/bulb_yellow.png\'>';
				}
				else if($realisasi_output >= 75 && $realisasi_output <= 100)
				{
					$warning_icon_output = '<img border=\'0\' src=\''.base_url().'images/flexigrid/bulb_green.png\'>';
				}
				else if($realisasi_output > 100)
				{
					$warning_icon_output = '<img border=\'0\' src=\''.base_url().'images/flexigrid/bulb_blue.png\'>';
				}

				//warning icon realisasi keuangan
				if($progres_output < 50)
				{
					$warning_icon_progres = '<img border=\'0\' src=\''.base_url().'images/flexigrid/bulb_red.png\'>';
				}
				else if($progres_output >= 50 && $progres_output < 75)
				{
					$warning_icon_progres = '<img border=\'0\' src=\''.base_url().'images/flexigrid/bulb_yellow.png\'>';
				}
				else if($progres_output >= 75 && $progres_output <= 100)
				{
					$warning_icon_progres = '<img border=\'0\' src=\''.base_url().'images/flexigrid/bulb_green.png\'>';
				}
				else if($progres_output > 100)
				{
					$warning_icon_progres = '<img border=\'0\' src=\''.base_url().'images/flexigrid/bulb_blue.png\'>';
				}
				...
				if($kd_role != Role_model::PEMBUAT_LAPORAN)
				{
					$record_items[] = array(
						...
						'<div style=\'text-align:left\'>'.'['.$kdgiat.']'.' '.$row->nmgiat.'</div>',
						'<div style=\'text-align:left\'>'.'['.$kdgiat.'.'.$kdoutput.']'.' '.$row->nmoutput.'</div>',
						'Rp '.number_format($alokasi),
						$rencana_fisik.' % ',
						$progress_fisik.' % ',
						$realisasi_fisik.' % '.$warning_icon_kontrak,
						$realisasi_keuangan.' % '.$warning_icon_keuangan,
						$realisasi_output.' % '.$warning_icon_output,
						$progres_output.' % '.$warning_icon_progres,
						//$masalah,
	...
	function save_progress_fisik($thang, $kdjendok, $kdsatker, $kddept, $kdunit, $kdprogram, $kdgiat, $kdoutput, $kdlokasi, $kdkabkota, $kddekon, $kdsoutput, $progress_id, $idpaket)
	{
		...
		redirect('e-monev/laporan_monitoring/input_progress/'.$thang.'/'.$kdjendok.'/'.$kdsatker.'/'.$kddept.'/'.$kdunit.'/'.$kdprogram.'/'.$kdgiat.'/'.$kdoutput.'/'.$kdlokasi.'/'.$kdkabkota.'/'.$kddekon.'/'.$kdsoutput);
	}
