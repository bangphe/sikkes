...
class Laporan_kinerja extends CI_Controller {
    ...
    function grid_data_kinerja($pilihan_prioritas = NULL) {
        ...
        foreach ($records['records']->result() as $row) {
			...
			$dja = $this->lkm->get_dja($row->KodeIkk);
			if ($dja->num_rows() > 0) {
			 	$realisasi_keuangan = $this->lkm->get_dja($row->KodeIkk)->row()->realisasi_keuangan;
			 	$realisasiSp2d = $this->lkm->get_dja($row->KodeIkk)->row()->realisasi_anggaran;
			 	$alokasiDipa = $this->lkm->get_dja($row->KodeIkk)->row()->pagu_anggaran;
			} else {
				$realisasi_keuangan = 0;
				$realisasiSp2d = 0;
				$alokasiDipa = 0;
			...
			if($kd_role != Role_model::PEMBUAT_LAPORAN) {
				$record_items[] = array(
					...
					'Rp '.number_format($realisasi_keuangan, 2), // realisasi keu
