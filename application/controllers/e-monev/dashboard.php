...
class Dashboard extends CI_Controller
{
		...
		else
		{
			...
			foreach($records->result_array() as $row){
				...
				foreach($this->dm->get_skmpnen_by_unit($thang, $row['KDUNIT'])->result_array() as $row1){
				  ...
				  if($pagu_unit > 0){
					$paket = $this->dm->get_paket($thang, $row1['kdjendok'], $row1['kdsatker'], $row1['kddept'], $row1['kdunit'], $row1['kdprogram'], $row1['kdgiat'], $row1['kdoutput'], $row1['kdsoutput'], $row1['kdkmpnen'], $row1['kdskmpnen']);
					if($paket->num_rows() > 0) {
					...
					if($prog_fis_swakelola > 0 && $prog_fis_kontraktual > 0) {
						...
						// realisasi fisik komponen unit utama 
						$fisik_unit = ($prog_fis_swakelola*$pagu_swakelola + $prog_fis_kontraktual*$pagu_kontraktual) / $pagu_unit;
					}
				  }	
					// filter flag warna prog fisik (merah kuning hijau biru)
					if($fisik_unit <= 50 ){
						$merah += 1;
					}elseif($fisik_unit >= 51 && $fisik_unit <= 75){
						$kuning += 1;
					}elseif($fisik_unit >= 76 && $fisik_unit <= 100){
						$hijau += 1;
					}elseif($fisik_unit > 100){
						$biru += 1;
					}
				}
	...
	function getProv($kdunit, $thang, $pagu_total){
		...
		$result = array();
		$records = $this->dm->get_prov_by_unit($kdunit);
		foreach($records->result_array() as $row){
			...
			// pagu total unit utama per provinsi
			$pagu_prov += $this->dm->get_pagu_prov_swakelola($thang, $kdunit, $row['KodeProvinsi'])->row()->jumlah;
			$pagu_prov += $this->dm->get_pagu_prov_kontraktual($thang, $kdunit, $row['KodeProvinsi'])->row()->nilaikontrak;
			
			foreach($this->dm->get_skmpnen_by_provinsi($thang, $kdunit, $row['KodeProvinsi'])->result_array() as $row1){
				$pagu_swakelola = 0;
				$pagu_kontraktual = 0;
				$prog_fis_swakelola = 0;
				$prog_fis_kontraktual = 0;
			...
			$jumlah_paket = $this->dm->count_paket_by_prov($kdunit, $row['KodeProvinsi'], $thang);
			$row['paket'] = $jumlah_paket.' PKT';
	...
	{
		$cek = $this->dm->get_jnssat_by_prov($kdunit, $kdlokasi);
		return $cek->num_rows() > 0 ? true : false;
	}
	...
	function getJnssat($kdunit, $kdlokasi, $thang, $pagu_total)
	{
		$bulan = date("n")-1;
		$result = array();
		$records = $this->dm->get_jnssat_by_prov($kdunit, $kdlokasi);
		foreach($records->result_array() as $row){
			...
			$pagu_jnssat += $this->dm->get_pagu_jnssat_swakelola($thang, $kdunit, $kdlokasi, $row['kdjnssat'])->row()->jumlah;
			$pagu_jnssat += $this->dm->get_pagu_jnssat_kontraktual($thang, $kdunit, $kdlokasi, $row['kdjnssat'])->row()->nilaikontrak;
			
			foreach($this->dm->get_skmpnen_by_jnssat($thang, $kdunit, $kdlokasi, $row['kdjnssat'])->result_array() as $row1){
				$pagu_swakelola = 0;
				$pagu_kontraktual = 0;
				$prog_fis_swakelola = 0;
				$prog_fis_kontraktual = 0;
			...
			$jumlah_paket = $this->dm->count_paket_by_jnssat($kdunit, $kdlokasi, $row['kdjnssat'], $thang);
			$row['paket'] = $jumlah_paket.' PKT';
	...
	{
		$cek = $this->dm->get_satker_by_jnssat($kdunit, $kdlokasi, $kdjnssat);
		return $cek->num_rows() > 0 ? true : false;
	}
	...
	function getSatker($kdunit, $kdlokasi, $kdjnssat, $thang, $pagu_total)
	{
		$bulan = date("n")-1;
		$result = array();
		$records = $this->dm->get_satker_by_jnssat($kdunit, $kdlokasi, $kdjnssat);
		...
		foreach($records->result_array() as $row){
			...
			// pagu total unit utama per jenis satker provinsi
			$pagu_satker += $this->dm->get_pagu_satker_swakelola($thang, $kdunit, $kdlokasi, $row['kdsatker'])->row()->jumlah;
			$pagu_satker += $this->dm->get_pagu_satker_kontraktual($thang, $kdunit, $kdlokasi, $row['kdsatker'])->row()->nilaikontrak;
			
			foreach($this->dm->get_skmpnen_by_satker($thang, $kdunit, $kdlokasi, $row['kdsatker'])->result_array() as $row1){
				$pagu_swakelola = 0;
				$pagu_kontraktual = 0;
				$prog_fis_swakelola = 0;
				$prog_fis_kontraktual = 0;
			...
			$jumlah_paket = $this->dm->count_paket_by_satker($kdunit, $kdlokasi, $row['kdsatker'], $thang);
			$row['paket'] = $jumlah_paket.' PKT';
