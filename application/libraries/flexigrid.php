...
class Flexigrid
{
	...
	*/	
	...
	public function validate_post($d_sortname,$d_sortorder,$valid_fields = NULL) 
	{
		...
		{
			//$var = explode(",",$d_sortname);
			//$this->post_info['sortname'] 	= $var[0];
			$this->post_info['sortname'] = $d_sortname;
			//$this->post_info['sortname2'] 	= $var[1];
			$this->post_info['sortorder'] 	= $d_sortorder;
		}
		...
		else
		{
			if (trim($searchstr) != "" && $searchby != "")
			{
				...
				$searchstr_final = "";
					...
					//kode sebelum 5 febuari
				if (trim($searchstr) != "")
					$searchstr_final .= $searchby.' LIKE "%'.$searchstr.'%"';
...
*/				
	...
	public function build_query($limit = TRUE)
	{
		...
		if($this->post_info['sortname'] != '') //case nya kalo order by lebih dari 1, maka order by dilaksanakan di query nya bukan dari flexigridnya
