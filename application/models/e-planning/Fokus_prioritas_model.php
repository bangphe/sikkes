<?php
class Fokus_prioritas_model extends CI_Model {
	/**
	 * Constructor
	 */
	...
	function get_fokus_prioritas(){
		$this->db->select('*');
		$this->db->from('fokus_prioritas');
		$this->db->join('ref_periode','fokus_prioritas.idPeriode=ref_periode.idPeriode');
		$this->db->order_by('FokusPrioritas', 'asc');
		$this->CI->flexigrid->build_query();		
		$return['records'] = $this->db->get();
