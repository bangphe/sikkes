<?php
class Mapping_output_model extends CI_Model {
	...
	function program()
	{
		...
		$this->db->select('t_program.kddept, t_program.kdprogram, t_program.nmprogram, d_item.kdunit, d_item.thang, d_item.kdjendok');
		...
		if($this->session->userdata('kd_role') == Role_model::PEMBUAT_ANGGARAN)
		{
	...
	function count_keg($kddept,$kdprogram)
	{
		$thang = $this->session->userdata('thn_anggaran');
		
		$this->db->select('COUNT(DISTINCT t_giat.kdgiat) as JUMLAH');
		$this->db->from('d_item');
		$this->db->join('t_giat', 'd_item.kdgiat = t_giat.kdgiat');
		$this->db->where('d_item.thang', $thang);
		$this->db->where('d_item.kdprogram', $kdprogram);
	...
	function output($kdprogram,$kdgiat)
	{
		...
		$this->db->from('d_item');
		$this->db->join('t_output', 'd_item.kdgiat = t_output.kdgiat AND d_item.kdoutput = t_output.kdoutput');
		//$this->db->join('d_soutput', 'd_soutput.kdgiat=t_output.kdgiat AND d_soutput.kdoutput=t_output.kdoutput');
		$this->db->where('d_item.thang', $thang);
		$this->db->where('d_item.kdprogram', $kdprogram);
	...
	function count_output($kdprogram,$kdgiat)
	{
		$thang = $this->session->userdata('thn_anggaran');
		
		$this->db->select('COUNT(DISTINCT t_output.kdoutput) as JUMLAH');
		$this->db->from('d_item');
		$this->db->join('t_output', 'd_item.kdgiat = t_output.kdgiat AND d_item.kdoutput = t_output.kdoutput');
		$this->db->where('d_item.thang', $thang);
		$this->db->where('d_item.kdprogram', $kdprogram);
    ...
    function save_fokus_prioritas($key, $ikk) {
        $ret = 0;
        ...
        $sql = "DELETE FROM data_m_output WHERE thang='$thang' AND kdunit='$kdunit' AND kdprogram='$kdprogram' AND kdgiat='$kdgiat' AND kdoutput='$kdoutput'";

        $result = mysql_query($sql) or die(mysql_error());

        $sql = "INSERT INTO data_m_output VALUES ('$thang', '$kdunit', '$kdprogram', '$kdgiat', '$kdoutput', '$ikk')";
        //die ($sql);
        $result = mysql_query($sql) or die(mysql_error());
    }

    function cek_mapping_by_program($thang,$kdunit,$kdprogram)
	{
		// $keys = explode("-", $key);
  //       $thang = $keys[0];
  //       $kdunit = $keys[1];
  //       $kdprogram = $keys[2];
		...
		$this->db->select();
		$this->db->from('data_m_output');
		$this->db->where('thang',$thang);
		$this->db->where('kdunit',$kdunit);
		$this->db->where('kdprogram',$kdprogram);
		$this->db->group_by('kdprogram');
		// $query = $this->db->get();
	...
	function cek_mapping_by_kegiatan($key)
	{
		$keys = explode("-", $key);
        $thang = $keys[0];
        $kdunit = $keys[1];
    ...
    function cek_mapping_output($key)
	{
		$keys = explode("-", $key);
        $thang = $keys[0];
        $kdunit = $keys[1];
