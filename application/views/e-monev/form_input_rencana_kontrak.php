...
function validasi_rencana(){
	var text = $("#rencana_kontrak").val();
	var rencana_kontrak = $("#rencana_kontrak_hidden").val();
	...
	if($.isNumeric(text) == false){
		...
		if(parseFloat(text) > 100){
			$("#rencana_kontrak").val(100);
			alert('Isian tidak boleh lebih dari 100.');
		}else if(parseFloat(text) < parseFloat(rencana_kontrak)){
			if(bulan != 'Januari'){
				$("#rencana_kontrak").val(rencana_kontrak);
			...
			if (text.indexOf(".") >= 0){
				...
				if(digit[1].length > 2){
					...
					$("#rencana_kontrak").val(num.toFixed(2));
				}else{
					$("#rencana_kontrak").val(text.substring(0, numofchars));
				}
			}
		}
	}
...
{
    $("#rencana_kontrak").unbind().keyup(function()
    {
        //check if the appropriate text area is being typed into
        if(document.activeElement.id === "rencana_kontrak")
        {
