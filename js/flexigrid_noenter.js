...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
			...
			doSearch: function () {
				p.query = $('input[name=query]',g.sDiv).val();
		...
		$('.pcontrol input',g.pDiv).keydown(function(e){ g.changePage('input')});
		...
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>"+p.findtext+" <input type='text' size='30' name='query' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <!--input type='button' value='Clear' /--></div>");

				$('input[name=query],select[name=qtype]',g.sDiv).keyup(function(e){g.doSearch()});
				$('input[value=Clear]',g.sDiv).click(function(){$('input[name=query]',g.sDiv).val(''); p.query = ''; g.doSearch(); });
				$(g.bDiv).after(g.sDiv);				
