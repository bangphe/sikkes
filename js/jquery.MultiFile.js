  ...
  options: {
   ...
   error: function(s){
    if($.blockUI){
     ...
     window.setTimeout($.unblockUI, 3500);
    }
    else{
     alert(s);
    }
   ...
   STRING: {
	//var a = 
    remove:'HAPUS',
    denied:'Anda tidak dapat memasukkan file dengan format $ext file.\n Gunakan file dengan format DB !',
    selected:'File selected: $file',
    duplicate:'File ini telah diseleksi, gunakan nama file lain:\n$file'
   }
  }
 });
   ...
   MultiFile: function( options /* Object */ ){
    ...
    return $(this).each(function(group_count){
     if(this._MultiFile) return;this._MultiFile = true;
       ...
       MF.E.wrap('<div class="inputupload" id="'+MF.wrapID+'" ></div>');
